...
function Reducer(state, action) {
  switch (action.type) {
    case 'GET_GRID_DATA': //search 컴포넌트에서 데이터를 받아 디스패치후 useReducer 기본 state에 데이터를 넣음
      return { ...state, gridCodeData: action?.gridCodeData };
    case 'CODE_DETAIL_RESET': // 데이터 제거시 모든 인풋 Value를 리셋 시킴
      return {  ...state,codeDetailData: {}};
    case 'CODE_DETAIL': // 그리드 테이블의 클릭한 유저 데이터를 받아 디테일 인풋 Value에 값을 채워 넣음.
      return { ...state, codeDetailData: { ...action?.codeDetail } };
...
export function CodeListContext({ children }) {
  const [state, dispatch] = useReducer(Reducer, CodeMgtState);

  return (
    <StateContext.Provider
      value={{
        gridCodeData: state?.gridCodeData,
        codeDetailData: state?.codeDetailData,
