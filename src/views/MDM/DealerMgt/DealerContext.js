...
function userReducer(state, action) {
  switch (action.type) {
    ...
    case 'DLR_DETAIL_RESET': // 데이터 제거시 모든 인풋 Value를 리셋 시킴
    return { ...state, dlrDetailData: {} };
    case 'DLR_DETAIL': // 그리드 테이블의 클릭한 유저 데이터를 받아 디테일 인풋 Value에 값을 채워 넣음.
      return { ...state, dlrDetailData: { ...action?.dlrDetail } };
...
export function DealerContext({ children }) {
  ...
  return (
    <StateContext.Provider
      value={{
        ...
        dlrDetailData: state?.dlrDetailData,
