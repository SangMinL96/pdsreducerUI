...
import { useDataDispatch, useDataState } from './MachineSttgContext';
import { useLazyQuery, useMutation } from '@apollo/react-hooks';
import { MachineSttg_DEL, GET_MachineSttg_DATA } from './MachineSttgQuery';
import { toast } from 'react-toastify';
import { variables} from "./MachineSttgComponent/MachineSttgUtils"
import { tableSelectDispatch,tableRemoveDispatch, selFocusDispatch} from 'src/components/dispatchs';
import { tableStyle } from './MachineSttgComponent/MachineSttgStyle';

const MachineSttgCloumns = [
  {
    headerName: '서비스',
    field: 'SVC_NM',
...
const MachineSttgTable = () => {
  ...
  const [removeMachineSttg, setRemoveMachineSttg] = useState([]); // 선택한 데이터를 배열로 모아 삭제
  const [delMutation] = useMutation(MachineSttg_DEL, { // 선택한 데이터 삭제
    variables: {
      MachineSttg: { GME_IDS: removeMachineSttg },
      ver: 'v1'
    },

  });
  const [refetch ] = useLazyQuery(GET_MachineSttg_DATA,variables({GME_NM:""}));
 ...
 */
  const onRowSelected=(ev)=>{
    ...
    setRemoveMachineSttg(clickNtcData);
  }
          ...
          onClick: async () => {
            try {
             ...
             const result = await delMutation();
              console.log(result)
             if(result.data.delMachineSttg.rsltCd==="OK"){
        ...
        <Box className={classes.TopBarBox}>
          ...
          <div>
              ...
              className={classes.button}
                ...
                variant="contained"
                color="primary"
                disabled={removeMachineSttg.length > 0 ? false :true}
              >
                삭제
              </Button>
...
MachineSttgTable.propTypes = {
  className: PropTypes.string
};

export default React.memo(MachineSttgTable);
