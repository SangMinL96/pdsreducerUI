...
const dlrState = {
  gridData: [],
  machineSttgDetailData: undefined,
  selClear: false,
  inputReadOnly: false,
  subMitFlag: {
    addFlag: false,
...
function userReducer(state, action) {
  switch (action.type) {
    ...
    case 'DETAIL_RESET': // 데이터 제거시 모든 인풋 Value를 리셋 시킴
    return { ...state, machineSttgDetailData: {} };
    case 'DETAIL_DATA': // 그리드 테이블의 클릭한 유저 데이터를 받아 디테일 인풋 Value에 값을 채워 넣음.
      return { ...state, machineSttgDetailData: { ...action?.detailData } };
...
export function MachineSttgContext({ children }) {
  ...
  return (
    <StateContext.Provider
      value={{
        ...
        machineSttgDetailData: state?.machineSttgDetailData,
