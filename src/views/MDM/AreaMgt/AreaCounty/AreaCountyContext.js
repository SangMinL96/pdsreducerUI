...
const AreaCountyState = {
  gridData: [],
  countyDetailData: undefined,
  selClear: false,
  inputReadOnly: false,
  subMitFlag: {
    addFlag: false,
...
function Reducer(state, action) {
  switch (action.type) {
    ...
    case 'COUNTY_DETAIL_RESET': // 데이터 제거시 모든 인풋 Value를 리셋 시킴
      return {  ...state,countyDetailData: {}};
    case 'COUNTY_DETAIL': // 그리드 테이블의 클릭한 유저 데이터를 받아 디테일 인풋 Value에 값을 채워 넣음.
      return { ...state, countyDetailData: { ...action?.countyDetail } };
...
export function AreaCountyContext({ children }) {
  const [state, dispatch] = useReducer(Reducer, AreaCountyState);

  return (
    <StateContext.Provider
      value={{
        gridData: state?.gridData,
        countyDetailData: state?.countyDetailData,
