...
import { GET_CITY_DATA, CITY_DEL } from './AreaCityQuery';
import { toast } from 'react-toastify';
import { tableRemoveDispatch, tableSelectDispatch} from "./AreaCityComponent/AreaCityUtils"
import { titleStyle } from './AreaCityComponent/AreaCityStyle';
...
const AreaCityTable = () => {
  ...
  const [delMutation] = useMutation(CITY_DEL, { // 선택한 데이터 삭제
    variables: {
      ver: 'v1',
      area:{AREA_IDS:removeCode},
    }
  });
  const { data,refetch,error } = useQuery(GET_CITY_DATA, {  // 데이터 삭제시 그리드 테이블 리프레쉬
    fetchPolicy: 'network-only',
    skip:city_urp_id ===undefined,
  ...
  }, [selClear,error,data,cityReset,globalDispatch, dispatch,selFocus,gridData,subMitFlag,gridApi]);
 ...
 */
  const onRowClickd = ev => {  tableSelectDispatch(ev,dispatch,globalDispatch) };
 ...
 */
  const onRowSelected=(ev)=>{
    ...
    const clickCodeData = gridApi.getSelectedRows().map(data => data.AREA_ID);
...
AreaCityTable.propTypes = {
  className: PropTypes.string
};

export default React.memo(AreaCityTable);
