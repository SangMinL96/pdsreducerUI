...
function userReducer(state, action) {
  switch (action.type) {
    ...
    case 'DETAIL_TABLE_DATA':
      return { ...state, detailTableData: action?.userId };
...
export function JoinUserContext({ children }) {
  const [state, dispatch] = useReducer(userReducer, screenState);
  ...
  return (
    <StateContext.Provider
      value={{
        ...
        detailTableData: state?.detailTableData,
