...
function apiReducer(state, action) {
  switch (action.type) {
    ...
    case 'API_DETAIL_RESET': // API 제거시 모든 인풋 Value를 리셋 시킴
      return {
        ...state,
        apiDetailData: { }
      };

    case 'API_DETAIL': // 그리드 테이블의 클릭한 API 데이터를 받아 디테일 인풋 Value에 값을 채워 넣음.
      return { ...state, apiDetailData: { ...action?.apiDetail } };
    ...
    case 'ADD_SUBMET': // 초기화 버튼 클릭시 addFlag를 true값으로 바꾸어 Submit 실행시 API 추가 함수가 실행 하도록함.
      return { ...state, subMitFlag: { addFlag: true, editFlag: false } };

    case 'EDIT_SUBMET': // 수정 버튼 클릭시 editFlag를 true값으로 바꾸어 Submit 실행시 API 수정 함수가 실행 하도록함.
    ...
    case 'EDIT_API': // API 수정시 수정할 API 데이터를 받아 API 정보 수정함
       return {...state,
         gridData: state?.gridData?.map(apis => {
           if (apis.API_ID === action?.editApiData?.API_ID) {
             return action?.editApiData;
           }
           return apis;
         })
       };
  ...
  return (
    <StateContext.Provider
      value={{
        ...
        apiDetailData: state?.apiDetailData,
