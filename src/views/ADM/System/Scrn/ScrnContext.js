...
function scrnReducer(state, action) {
  switch (action.type) {
    case 'GET_GRID_DATA': //Bar 컴포넌트에서 데이터를 받아 디스패치후 useReducer 기본 state에 데이터를 넣음
      return { ...state, gridData: action?.gridData };
    case 'SCRN_DETAIL_RESET': // SCRN 제거시 모든 인풋 Value를 리셋 시킴
      return {
        ...state,
        scrnDetailData: { }
      };

    case 'SCRN_DETAIL': // 그리드 테이블의 클릭한 SCRN 데이터를 받아 디테일 인풋 Value에 값을 채워 넣음.
      return { ...state, scrnDetailData: { ...action?.scrnDetail } };
    ...
    case 'ADD_SUBMET': // 초기화 버튼 클릭시 addFlag를 true값으로 바꾸어 Submit 실행시 SCRN 추가 함수가 실행 하도록함.
      return { ...state, subMitFlag: { addFlag: true, editFlag: false } };

    case 'EDIT_SUBMET': // 수정 버튼 클릭시 editFlag를 true값으로 바꾸어 Submit 실행시 SCRN 수정 함수가 실행 하도록함.
    ...
    case 'EDIT_SCRN': // SCRN 수정시 수정할 SCRN 데이터를 받아 SCRN 정보 수정함
       return {...state,
         gridData: state?.gridData?.map(scrns => {
           if (scrns.SCRN_ID === action?.editScrnData?.SCRN_ID) {
             return action?.editScrnData;
           }
           return scrns;
         })
       };
...
export function ScrnContext({ children }) {
  const [state, dispatch] = useReducer(scrnReducer, scrnState);

  return (
    <StateContext.Provider
      value={{
        gridData: state?.gridData,
        scrnDetailData: state?.scrnDetailData,
